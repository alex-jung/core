...
def line_hash(line: Line | None) -> str:
...
def compare_list_lines(list_1: list[Line], list_2: list[Line | None]) -> bool:
    """Compares two lists of lines."""
    if len(list_1) != len(list_2):
        return False

    list_1_hashes = [line_hash(x) for x in list_1]
    list_2_hashes = [line_hash(x) for x in list_2]

    return set(list_1_hashes) == set(list_2_hashes)
